module.exports = function(app, passport, db, multer, ObjectId) {
 ...
 // normal routes ===============================================================
     ...
     app.post('/makePost', upload.single('file-to-upload'), (req, res) => {
      ...
      db.collection('posts').save({caption: req.body.caption, star: 0, img: 'images/uploads/' + req.file.filename, postedBy: user}, (err, result) => {
        if (err) return console.log(err)
 ...
 app.get('/post/:zebra', isLoggedIn, function(req, res) {
  ...
  db.collection('posts').find({postId}).toArray((err, result) => {
    if (err) return console.log(err)
    res.render('post.ejs', {
      posts: result
    })
...
app.post('/post/comment', (req, res) => {
  let user = req.user._id
  db.collection('comments').save({comment: req.body.comment, commentBy: user, postId: req.body.postId}, (err, result) => {
    if (err) return console.log(err)
    console.log('saved to database')
 ...
 app.get('/page/:id', isLoggedIn, function(req, res) {
   var postId = ObjectId(req.params.id)
